  ...
  return <div className="modal-backdrop" onClick={loading ? undefined : onClose}>
    <div className="modal-card" style={{ maxWidth: '720px', maxHeight: '90vh', overflowY: 'auto' }} onClick={(event) => event.stopPropagation()}>
      ...
      <form onSubmit={submit}>
        <div className="form-field" style={{ marginBottom: '0.85rem' }}><label>Correo del destinatario *</label><input className="input-control" type="email" placeholder="usuario@correo.com" value={recipient} onChange={(e) => setRecipient(e.target.value)} disabled={loading} required /></div>
        <div className="form-field" style={{ marginBottom: '0.85rem' }}><label>Alcance *</label><select className="input-control select-control" value={scope} onChange={(e) => setScope(e.target.value)} disabled={loading}><option value="vault">Toda la bóveda</option><option value="file">Un archivo</option></select></div>
        {scope === 'file' && <div className="form-field" style={{ marginBottom: '0.85rem' }}><label>ID de archivo *</label><input className="input-control" placeholder="ID del archivo" value={fileId} onChange={(e) => setFileId(e.target.value)} disabled={loading} required /></div>}
        <div className="form-field" style={{ marginBottom: '0.85rem' }}><label>Expira el</label><input className="input-control" type="datetime-local" value={expiresAt} onChange={(e) => setExpiresAt(e.target.value)} disabled={loading} /></div>
        <p style={{ fontSize: '.8rem', color: 'var(--text-muted)', marginBottom: '0.85rem' }}>Pegá el sobre cifrado creado localmente para la identidad pública del dispositivo destinatario. El servidor solo lo conserva y valida la identidad del dispositivo.</p>
        {Object.entries(envelope).map(([key, value]) => <div className="form-field" style={{ marginBottom: '0.85rem' }} key={key}><label>{key.replaceAll('_', ' ')}</label><input className="input-control" value={value} onChange={(e) => updateEnvelope(key, e.target.value)} disabled={loading} required /></div>)}
        <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: '1rem' }}><button className="btn btn-primary" disabled={loading}>{loading ? 'Guardando...' : 'Crear acceso temporal'}</button></div>
      </form>
      ...
      {confirming && <div className="alert-banner error" style={{ marginTop: '1rem' }}><div style={{ flex: 1 }}><strong>Confirmar revocación</strong><p>El acceso y sus sobres quedarán revocados. Las sesiones del destinatario se cerrarán inmediatamente.</p><input className="input-control" placeholder="Motivo opcional" value={reason} onChange={(e) => setReason(e.target.value)} disabled={loading} style={{ marginTop: '0.5rem' }} /></div><button type="button" className="btn btn-primary" onClick={revoke} disabled={loading}>Confirmar</button><button type="button" className="btn btn-secondary" onClick={() => setConfirming(null)} disabled={loading}>Cancelar</button></div>}
    </div>
  </div>;
}
